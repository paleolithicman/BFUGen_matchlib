...
class flow_table_read : public sc_module {
public:
    sc_in<bool>                      CCS_INIT_S1(i_clk);
    sc_in<bool>                      CCS_INIT_S1(i_rst);

    bfu_in::slave<>                  CCS_INIT_S1(flow_table_read_in);
    bfu518_out::master<>             CCS_INIT_S1(flow_table_read_out);

    SC_CTOR(flow_table_read) {
        SC_THREAD(th_run);
        sensitive << i_clk.pos();
        async_reset_signal_is(i_rst, true);
    }
...
class flow_table_write : public sc_module {
public:
    sc_in<bool>                      CCS_INIT_S1(i_clk);
    sc_in<bool>                      CCS_INIT_S1(i_rst);
...
public:
    ...
    void th_run() {
        ...
        infile.open("/home/rui.ma/primate/BFUGen_matchlib/pktReassembly/input.txt");
...
public:
    CCS_DESIGN(pktReassembly) CCS_INIT_S1(pktReassembly_inst);
    // Module declarations
    source CCS_INIT_S1(source_inst);
    sink CCS_INIT_S1(sink_inst);
    flow_table_read CCS_INIT_S1(flow_table_read_inst);
    flow_table_write CCS_INIT_S1(flow_table_write_inst);
    ...
    SC_CTOR(Top) : clk_sig("clk_sig", 4, SC_NS) {
